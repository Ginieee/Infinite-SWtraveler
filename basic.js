class App {
    ...
    _setAnimationSpeed(speed) {
        for (const key in this._animationMap) {
            this._animationMap[key].setEffectiveTimeScale(speed);
        }
    }
    ...
    _processAnimation() {
        ...
        if (this._pressedKeys["w"] || this._pressedKeys["a"] || this._pressedKeys["s"] || this._pressedKeys["d"]) {
            ...
            if (this._pressedKeys["shift"]) {
                this._setAnimationSpeed(2.5); // 애니메이션 속도를 2.0으로 설정
        ...
        } else if (this._pressedKeys["1"]) {
            this._currentAnimationAction = this._animationMap["Jump_1"];

        } else if (this._pressedKeys["2"]) {
            this._currentAnimationAction = this._animationMap["Jump_3"];

        } else if (this._pressedKeys["x"]) {
            this._currentAnimationAction = this._animationMap["Study"];
    ...
    _setupModel() {
        ...
        new THREE.GLTFLoader().load("data/mudang_simple.glb", (gltf) => {
    ...
    update(time) {
        ...
        if (this._mixer) {
            ...
            if (this._isHold && this._model.position.x < -800) {
                ...
                if (this._model.position.z > 500 && this._model.position.x > 500) {
                    this._cameraHold.x = 120
                    this._cameraHold.y = 240;
                    this._cameraHold.z = 240;
                ...
                this._cameraHold.set(-200, 500, 500);
            }
        ...
        if (this._model && this._gachonBuilding) {
            const distance = this._model.position.distanceTo(this._gachonBuilding.position);
            if (distance < 700) {
                this._cameraHold.set(100, 240, 200);
            }
        }
